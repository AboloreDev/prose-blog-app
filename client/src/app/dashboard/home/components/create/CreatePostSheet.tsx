...
export const CreatePostSheet = () => {
  ...
  const preselectedCommunityId = useSelector(
    (state: RootState) => state.posts.preselectedCommunityId,
  );

  const userId = useSelector((state: RootState) => state.auth.user?.id);

  const { data: communities = [], isLoading: communitiesLoading } =
    useGetUserCommunitiesQuery({ id: userId! });
  ...
  useEffect(() => {
    if (isSheetOpen && preselectedCommunityId) {
      form.setValue("community_id", preselectedCommunityId, {
        shouldValidate: true,
      });
    }
      ...
      >
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          <CommunitySelector
            ...
            error={form.formState.errors.community_id?.message}
            readOnly={!!preselectedCommunityId}
            readOnlyName={
              preselectedCommunityId
                ? communities.find((c) => c.id === preselectedCommunityId)?.name
                : undefined
            }
          />
          ...
          <div className="flex gap-3 pt-2">
            <button
              type="button"
              className="flex-1 rounded-full bg-white text-black px-4 py-2"
