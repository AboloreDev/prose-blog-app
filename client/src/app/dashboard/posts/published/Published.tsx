...
import EditPostsSheet from "../EditPostsSheet";
...
const Drafts = () => {
  ...
  const { data, isLoading, isFetching, isError } = useGetUserPostsQuery({
    page,
    page_size: 10,
    query: searchQuery,
    user_id: user?.id ?? 0,
  ...
  const publishedPosts = ((data as any)?.UserPosts ??
    data?.Posts ??
    []) as Post[];
  const totalPublishedPosts = metadata?.total_records ?? publishedPosts.length;
  ...
  return (
    <section className="h-screen overflow-hidden lato-regular flex flex-col">
      <Header
        title="Drafts"
        subTitle={
          !isLoading && !isError && totalPublishedPosts > 0
            ? `${totalPublishedPosts} saved draft${totalPublishedPosts !== 1 ? "s" : ""}`
        ...
        <div className="max-w-3xl mx-auto">
          ...
          {isError && !isLoading && (
            <div className="flex flex-col items-center justify-center py-24 gap-4">
              ...
              <p className="text-sm text-muted-foreground">
                Failed to load published posts. Try refreshing.
              </p>
            </div>
          )}
            ...
            <div className="flex flex-col items-center justify-center py-24 gap-4">
              ...
              <div className="text-center space-y-1">
                ...
                <p className="text-sm text-muted-foreground max-w-xs">
                  Looks like you haven't posted in a while. Start writing and
                  publishing.
                </p>
              </div>
              <Button
      ...
      <CreatePostSheet />
      {/* Rendered once — shared across all cards */}
      <EditPostsSheet />
      <DeletePostModal />
    </section>
  );
